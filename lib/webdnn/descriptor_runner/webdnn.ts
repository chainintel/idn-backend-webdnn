...
import {
  DescriptorRunner as DescriptorRunnerGeneric,
  DescriptorRunnerConstructor
} from './descriptor_runner/descriptor_runner';
...
const descriptorRunners: {
  [k in BackendName]: DescriptorRunnerConstructor<GraphDescriptor, any>
} = {
...
export function getBackendAvailability(): BackendAvailability {
  ...
  let order = (['webgpu', 'webgl', 'webassembly', 'fallback'] as BackendName[]).filter(
...
export async function load(
  ...
  while (backendOrder.length > 0) {
    ...
    try {
      ...
      switch (cacheStrategy) {
        case 'latest':
          ...
          if (
            cachedDescriptor &&
            fetchedDescriptor &&
            cachedDescriptor.converted_at === fetchedDescriptor.converted_at
          ) {
        ...
        default:
          throw Error(
            `"${cacheStrategy}" is not valid cache strategy name: "latest", "networkFirst", "networkOnly", "cacheFirst", "cacheOnly" is available.`
          );
      }
      ...
      console.warn(
        `Model loading failed for ${backendName} backend. Trying next backend: ${ex.message}`
      );
      continue;
    }
    return runner;
