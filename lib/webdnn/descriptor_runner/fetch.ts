 ...
 * @protected
 */
export default async function webdnnFetch(
  input: RequestInfo,
  transformUrlDelegate: (base: string) => string,
  init?: WebDNNRequestInit
) {
 ...
 * @protected
 */
export function readArrayBufferProgressively(
  res: Response,
  callback?: (loaded: number, total: number) => any
): Promise<ArrayBuffer> {
...
function fetchUsingXHR(url, callback): Promise<Response> {
  return new Promise(function(resolve, reject) {
    ...
    req.onprogress = function(event) {
      if (callback) {
        callbackScheduler.request(function() {
          return callback(event.loaded, event.total);
        });
      }
    };
