...
> {
  ...
  async fetchDescriptor(directory: string) {
    let res = await webdnnFetch(
      `${directory}/graph_${this.backendName}_${this.handler.MAX_TEXTURE_SIZE}.json`,
      this.transformUrlDelegate
    );
    return res.json();
  }
  ...
  async fetchParameters(
    ...
    let res = await webdnnFetch(
      `${directory}/weight_${this.backendName}_${this.handler.MAX_TEXTURE_SIZE}.bin`,
      this.transformUrlDelegate
    );
  ...
  async restoreCachedDescriptor(directory: string): Promise<GraphDescriptorWebGL | null> {
    return localforage
      .getItem<GraphDescriptorWebGL>(
        `${directory}_${this.backendName}_${this.handler.MAX_TEXTURE_SIZE}_descriptor`
      )
      .catch(() => null);
  }
   ...
   * @protected
   */
  async restoreCachedParameters(
    directory: string,
    progressCallback?: (loaded: number, total: number) => any
  ): Promise<ArrayBuffer | null> {
    let parameter = await localforage
      .getItem<ArrayBuffer>(
        `${directory}_${this.backendName}_${this.handler.MAX_TEXTURE_SIZE}_parameters`
      )
      .catch(() => null);
  ...
  private async initializeStaticBuffer(weightRawArray: ArrayBuffer) {
    ...
    Object.entries(descriptor.memory_layout.static.allocations).forEach(
      ([name, { width, height, size, channel_mode }]) => {
        buffers.set(
          name,
          new BufferWebGL(
    ...
    Object.entries(descriptor.constants_map).forEach(([name, { size, byte_offset }]) => {
      buffers.get(name)!.array.set(new Float32Array(weight.buffer, byte_offset, size));
    });

    (await this.getInputViews()).filter((view) => !view.isDynamic).forEach((view) => {
      view.buffer = buffers
        .get(mapping[view.name])!
        .getWriteView(0, view.length, Float32Array).buffer;
    });

    (await this.getOutputViews()).filter((view) => !view.isDynamic).forEach((view) => {
      view.buffer = buffers
        .get(mapping[view.name])!
        .getReadView(0, view.length, Float32Array).buffer;
    });
  }
  ...
  private async initializeDynamicBuffer() {
    ...
    Object.entries(descriptor.memory_layout.dynamic.allocations).forEach(
      ([name, { width, height, size, channel_mode }]) => {
        buffers.set(
          name,
          new BufferWebGL(
            placeholderContext.resolve(size) * Float32Array.BYTES_PER_ELEMENT,
            placeholderContext.resolve(width),
            placeholderContext.resolve(height),
            name,
            null,
            channel_mode
          )
    ...
    (await this.getInputViews()).filter((view) => view.isDynamic).forEach((view) => {
      view.buffer = buffers
        .get(mapping[view.name])!
        .getWriteView(0, placeholderContext.resolve(view.length), Float32Array).buffer;
    });

    (await this.getOutputViews()).filter((view) => view.isDynamic).forEach((view) => {
      view.buffer = buffers
        .get(mapping[view.name])!
        .getReadView(0, placeholderContext.resolve(view.length), Float32Array).buffer;
    });
  ...
  private async compile() {
    ...
    Object.keys(descriptor.shader_sources).forEach((name) => {
    ...
    if (this.runtimeInfo.programs.length > 0) {
      ...
      if (getConfiguration('DEBUG', false)) {
        ...
        for (let runtimeProgramInfo of runtimeInfo.programs) {
          ...
          this.handler.bindFrameBuffer(
            runtimeProgramInfo.frameBuffer,
            runtimeProgramInfo.width,
            runtimeProgramInfo.height
          );
        ...
        summary.forEach(
          (record) =>
            (record['Ratio [%]'] = (record['Elapsed time [ms]'] / totalElapsedTime).toFixed(2))
        );
