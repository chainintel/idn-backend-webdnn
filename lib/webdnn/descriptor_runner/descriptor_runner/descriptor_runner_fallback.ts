...
> {
  ...
  async fetchDescriptor(directory: string) {
    ...
    let res = await webdnnFetch(
      `${directory}/graph_${this.backendName}.json`,
  ...
  async fetchParameters(
    ...
    let res = await webdnnFetch(
      `${directory}/weight_${this.backendName}.bin`,
      this.transformUrlDelegate
    );
  ...
  async restoreCachedDescriptor(directory: string): Promise<GraphDescriptorFallback | null> {
    return localforage
      .getItem<GraphDescriptorFallback>(`${directory}_${this.backendName}_descriptor`)
      .catch(() => null);
  }
  ...
  async restoreCachedParameters(
    ...
    let parameter = await localforage
      .getItem<ArrayBuffer>(`${directory}_${this.backendName}_parameters`)
      .catch(() => null);
  ...
  private async compile(): Promise<void> {
    ...
    this.kernelObj =
      (window as any).dnn_fallback_kernel || ((global as any).window as any).dnn_fallback_kernel; // "(window).dnn_fallback_kernel" is defined in "kernels_fallback.js"
  }
  ...
  private async initializeStaticBuffer(weightRawArray: ArrayBuffer) {
    ...
    Object.entries(descriptor.memory_layout.static.allocations).forEach(
      ([name, allocation]: [string, ResolvedAllocation]) => {
        variableMap.set(
          name,
          new Float32Array(
            staticBuffer.buffer,
  ...
  private async initializeDynamicBuffer() {
    ...
    let dynamicBuffer = new Float32Array(
      placeholderContext.resolve(descriptor.memory_layout.dynamic.size)
    );
    ...
    Object.entries(descriptor.memory_layout.dynamic.allocations).forEach(
      ([name, allocation]: [string, Allocation]) => {
        variableMap.set(
          name,
          new Float32Array(
  ...
  async run(): Promise<void> {
    ...
    let placeholderContext = this.placeholderContext;
    let executionInfos = this.descriptor.exec_infos.map((executionInfo) =>
      placeholderContext.resolve(executionInfo)
    );
    ...
    for (let i = 0; i < executionInfos.length; i++) {
      ...
      if (currentDate - lastDate >= 1000) {
        console.log(
          `Processed ${i}/${executionInfos.length} kernels in ${currentDate - startDate} ms`
        );
    ...
    console.log(
      `Processed ${executionInfos.length}/${executionInfos.length} kernels in ${Date.now() -
        startDate} ms`
    );
  }
  ...
  getInputViews() {
    ...
    this.inputs = descriptor.inputs.map((name) => {
      let allocation =
        descriptor.memory_layout.static.allocations[name] ||
        descriptor.memory_layout.dynamic.allocations[name];
