...
> {
  ...
  async setDescriptorAndParameters(
  ...
  async restoreCachedDescriptor(directory: string): Promise<GraphDescriptorWebassembly | null> {
    this.directory = directory;
    return localforage
      .getItem<GraphDescriptorWebassembly>(`${directory}_${this.backendName}_descriptor`)
      .catch(() => null);
  }
  ...
  async setPlaceholderValue(values: { [key: string]: number }): Promise<void> {
    ...
    let dynamicBufferSize = this.placeholderContext.resolve(
      this.descriptor.memory_layout.dynamic.size
    );
  ...
  private setPlaceholderValueWorker(
    ...
    metaBufferFillArray: Int32Array
  ): Promise<void> {
  ...
  private async loadWeights(weightsData: Uint8Array) {
    ...
    let promise = new Promise<void>((resolve, reject) => {
      ...
      worker.postMessage({ type: 'weight', data: weight_data }, [weight_data.buffer] as any[]);
    });
  ...
  async run(): Promise<void> {
    ...
    let promise = new Promise<void>((resolve, reject) => {
      ...
      let allocations = [
        descriptor.memory_layout.static.allocations,
        descriptor.memory_layout.dynamic.allocations
      ];
      let inputs: any = [];
